...
#	No tuve tiempo de hacer andar esta implementacion
#  lo solucione multiplicando la velocidad.x *-1 en el timer
#  se que no es lo optimo, en una situacion mas compleja no sirve.

#para probar este bloque, primero comentar lineas 69 30 103
#	if !path.empty():
#		var next_point:Vector2 = path.front()
#		while path.size() > 1 && position.distance_to(next_point) < 3:
#			path.pop_front()
#			next_point = path.front()
#
#		if position.distance_to(next_point) > 3:
